...
vector<Token> Lexer::LexerAritmetico(string text)
{
    ...
    for (char cChar : text)
    {
        ...
        case '#':
            if (cToken.sType == Whitespace)
            {
                cToken.sType = Logico_posible;
                cToken.sText.append(1, cChar);
                break;
            }
        case 't':
        case 'f':
            if (cToken.sType == Logico_posible)
            {
                cToken.sType = Logico;
                cToken.sText.append(1, cChar);
                break;
...
void Lexer::addToken(Token &token, vector<Token> &tokens)
{
    ...
    else if (token.sText == "define")
    {
        token.sType = Reservada;
        tokens.push_back(token);
    }
...
void Token::lPrint(string arr[2])
{
    arr[0] = sText;
    arr[1] = TypeLArray[sType];
    //return
